...
import static com.huyehya.bot.loteria.api.DTOs.GameEnum.MEGASENA;

public class MegaSenaStrategy implements Rule {
    ...
    public GameDTO generator(final int amountOfGames){
        ...
        for (int quantity = 1; quantity <= amountOfGames; quantity++){
            ...
            while(results.size() <= 5){
                final int number = random.nextInt(MEGASENA.getTotalNumber()) + 1;
                results.add(number);
            }
    ...
    public GameDTO buildGame(final Set<CombinationDTO> combinationDTOs){
        return GameDTO.builder().name(MEGASENA.getName()).combinations(combinationDTOs).build();
    }
}
